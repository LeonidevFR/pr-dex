 ...
 */
/**
 * Le classement et les badges vivaient ici, en onglet. Ils montraient un sous-ensemble de ce que
 * la page Saison montre désormais — sans le compte à rebours, le badge en jeu, le barème ni
 * l'étagère — et deux écrans qui disent la même chose finissent par ne plus la dire pareil.
 * Leurs cas ont suivi le contenu : ils vivent dans `SeasonPanel.test.js`.
 */
describe('la saison n’est plus dans l’arène', () => {
  it('ne double plus le classement', async () => {
    const w = await monter()
    expect(w.text()).not.toContain('Classement')
    expect(w.findAll('.arena-badge')).toHaveLength(0)
  })

  // On y renvoie plutôt que de la résumer : sans ce renvoi, un joueur qui cherche son rang
  // depuis l'arène ne saurait pas où aller.
  it('renvoie vers l’écran qui la porte', async () => {
    expect((await monter()).text()).toContain('Saison')
  })
})
