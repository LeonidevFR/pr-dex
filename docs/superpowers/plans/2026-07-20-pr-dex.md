...
- **Trois trous identifiés dans les specs, sciemment non corrigés** (arbitrage du propriétaire, juillet 2026 — à documenter comme dette, pas comme oubli) :
  1. *Un fichier illisible est diagnostiqué comme une panne GitHub.* Si `state.json` ou `catches.json` contient du JSON invalide, l'API répond 200 mais `JSON.parse` lève une `SyntaxError`, qui n'est pas une `GithubError` : le code retombe sur `kind: 'server'` et l'écran affiche « GitHub est indisponible, réessaie dans un moment ». Réessayer ne répare jamais ce cas. Contraire à l'esprit de la table des modes de défaillance de la spec, qui exige un diagnostic exact et un chemin de réparation. Correctif : un `kind: 'corrupt'` distinct, avec un message nommant le fichier.
  2. *« Tout ouvrir sans cérémonie » écrit une fois par pli.* `skipAll` boucle sur `claim()`, donc N allers-retours API et N commits. À vingt PR en attente, plusieurs secondes sans retour visuel et un état partiel si l'une échoue. Correctif : une seule écriture groupée.
  3. *L'Action refait les appels de détail à chaque exécution.* La spec présente la déduplication sur `sha` comme seule protection d'idempotence, mais le `sha` n'est connu qu'après le `GET /pulls/{n}` : il ne peut donc pas éviter cet appel. Toutes les 30 minutes, l'Action re-interroge toutes les PR de la fenêtre de sept jours (~300 appels/jour pour un seul utile). Correctif : dédupliquer sur `(repo, pr)` — tous deux déjà stockés — avant l'appel de détail, en gardant la déduplication sur `sha` comme garde-fou final.
