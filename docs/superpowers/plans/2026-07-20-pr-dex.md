...
const shaAt = (i) =>
  Array.from({ length: 40 }, (_, k) => '0123456789abcdef'[fnv1a('sha' + i + '_' + k) % 16]).join('')
...
const shaAt = i => Array.from({length:40},(_,k)=>'0123456789abcdef'[fnv1a('sha'+i+'_'+k)%16]).join('')
...
describe('bonbons', () => {
  it('crédite trois bonbons par capture à la famille', () => {
    ...
    expect(d.candies.value(1)).toBe(6) // Herbizarre + Florizarre → famille Bulbizarre
  })
  ...
  it('déduit les bonbons dépensés', () => {
    ...
    expect(d.candies.value(1)).toBe(1) // 9 gagnés − 8 dépensés
  })
...
describe('évolution possible', () => {
  it('autorise l’évolution quand les bonbons suffisent', () => {
    ...
    expect(d.canEvolve.value(1)).toBe(true) // 9 ≥ 8
  })

  it('refuse l’évolution en dessous du coût', () => {
    const d = setup([catchOf('a', 1)], { claimed: ['a'] })
    expect(d.canEvolve.value(1)).toBe(false) // 3 < 8
  })
...
describe('bonbons morts', () => {
  it('repère une espèce dont la famille n’évolue pas', () => {
    const d = setup([], {})
    expect(d.isDeadEnd.value(143)).toBe(true)  // Ronflex
...
describe('évolution', () => {
  ...
  it('dépense les bonbons et enregistre l’évolution', async () => {
    ...
    expect(c.state.value.evolutions).toEqual([{ species: 2, from: 1, date: '2026-07-20' }])
    expect(client.writeState).toHaveBeenCalledOnce()
  })
...
export function useCollection() {
  ...
  async function evolve(fromId, toId, date) {
    ...
    if (!dex.canEvolve.value(fromId)) return
    ...
    await persist(
      (s) => ({
        ...
        evolutions: [...s.evolutions, { species: toId, from: fromId, date }],
      }),
...
const fakeSha = (i) =>
  Array.from({ length: 40 }, (_, k) => '0123456789abcdef'[fnv1a('seed' + i + '_' + k) % 16]).join('')
...
<transition name="fade">
  <SpeciesSheet
    ...
    :candies="collection.dex.candies.value(selected)"
    :can-evolve="collection.dex.canEvolve.value(selected)"
    :is-dead-end="collection.dex.isDeadEnd.value(selected)"
    @close="selected = null"
    @evolve="onEvolve"
  />
</transition>
